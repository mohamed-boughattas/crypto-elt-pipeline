...
- **pip-audit/bandit**: `continue-on-error: true` in CI — they report unfixable transitive vulnerabilities from airbyte's deps (documented in `.pip-audit.toml`).
...
- **CI job chain**: `lint` and `test` run in parallel → `dbt` job runs after both pass. `security` job runs independently.
